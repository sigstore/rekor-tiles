...
use `log2026-1` as an example for this document.
...
Note that `network_endpoint_group_zones` needs to remain commented out, as GKE
will create the network endpoint group (NEG) resources after we apply a Helm chart.
...
4. Revoke IAM access by reverting the PR and removing `google_kms_key_ring_iam_member`, and run `plan` and `apply`.
...
the values accordingly based on the Terraform resources names. In particular, update the `shardName`.
...
Tracked in https://github.com/sigstore/rekor-tiles/issues/46. We're aiming to avoid
needing to manually update the prober. The prober should pick up new shards from the
SigningConfig, with the latest active shard having no end date. Only the active
shard will accept write traffic. All other shards in the SigningConfig can still
handle read traffic.
...
You will need two timestamps:

* New shard's start for SigningConfig, which should be one week past when the root-signing PR is merged
...
if they are after this date. This must be after the start of the new shard. It doesn't have to
...
They key will be printed as a PEM-encoded public key. Remove the PEM header and footer and remove
...
Pass `--read-only=true` to the service, which will stop write traffic and
return an error to the caller. In the previous shard under `rekorTiles.shards`,
add:
...
### Verify write traffic again

Write traffic should now be exactly 0 QPS.
...
to `true`. Create a PR, merge, `plan`, and `apply`.
...
`keyring_name_suffix`, `key_name` and `kms_crypto_key_algorithm`. All other variables
should remain, for the networking stack to serve read traffic.

WIP: We also need to tear down the Kubernetes NEG backends for write traffic.
