...
// WriteClient writes entries to rekor.
...
func NewWriter(writeURL, origin string, verifier signature.Verifier, opts ...Option) (WriteClient, error) {
	cfg := &clientConfig{}
	for _, o := range opts {
		o(cfg)
	}
	...
	httpClient := &http.Client{
		Transport: createRoundTripper(http.DefaultTransport, cfg.userAgent, nil),
		Timeout:   cfg.timeout,
	}
	return &writeClient{
		baseURL:  baseURL,
