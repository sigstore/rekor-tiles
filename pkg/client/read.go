...
// ReadClient reads checkpoints, tiles, and entry bundles from the tile storage service.
...
// (This is currently specific to GCS storage).
func NewReader(readURL, origin string, verifier signature.Verifier, opts ...Option) (ReadClient, error) {
	cfg := &clientConfig{}
	for _, o := range opts {
		o(cfg)
	}
	...
	httpClient := &http.Client{
		Transport: createRoundTripper(http.DefaultTransport, cfg.userAgent, url.Values{"alt": []string{"media"}}),
		Timeout:   cfg.timeout,
	}
