...
func (s *Storage) Add(ctx context.Context, entry *tessera.Entry) (*rekor_pb.TransparencyLogEntry, error) {
	...
	inclusionProof, err := s.buildProof(ctx, idx, checkpointBody)
	if err != nil {
...
func (s *Storage) buildProof(ctx context.Context, idx *safeInt64, signedCheckpoint []byte) (*rekor_pb.InclusionProof, error) {
	...
	proof, err := proofBuilder.InclusionProof(ctx, idx.u)
	if err != nil {
