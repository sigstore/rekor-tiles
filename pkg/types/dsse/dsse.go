...
// to verify the envelope's signatures. A list of signatures mapped to their verifiers is returned, along with
// all possible hash algorithms to use for hashing the payload.
func verifyEnvelope(verifiers map[*pb.Verifier]verifier.Verifier, pbenv *pbdsse.Envelope, algorithmRegistry *signature.AlgorithmRegistryConfig) ([]*pb.Signature, map[crypto.Hash]v1.HashAlgorithm, error) {
	env := FromProto(pbenv)
