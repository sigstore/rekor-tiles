...
var serveCmd = &cobra.Command{
	...
	Run: func(cmd *cobra.Command, _ []string) {
		...
		switch signerType := viper.GetString("signer-type"); signerType {
		case "file":
		...
		default:
			slog.Error(fmt.Sprintf("unrecognized value for --signer-type: %s", signerType))
			os.Exit(1)
		}
...
func init() {
	...
	serveCmd.Flags().String("signer-type", "file", "type of signer to use, must be one of 'file', 'kms', or 'tink'")
