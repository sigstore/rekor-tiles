...
func NewIdentityServer(storage tessera.Storage, readOnly bool, algorithmRegistry *signature.AlgorithmRegistryConfig, logID []byte) *Server {
	if readOnly {
		return &Server{
			readOnly:     readOnly,
			logID:        logID,
...
func (s *Server) CreateEntry(ctx context.Context, req *pb.CreateEntryRequest) (*pbs.TransparencyLogEntry, error) {
	if s.identityMode {
		return nil, status.Errorf(codes.Unimplemented, "server is running in identity mode; use CreateIdentityEntry instead")
	}
	if s.readOnly {
...
func (s *Server) CreateIdentityEntry(ctx context.Context, req *pb.IdentityRequestV001) (*httpbody.HttpBody, error) {
	if !s.identityMode {
		return nil, status.Errorf(codes.Unimplemented, "server is not running in identity mode")
	}
	if s.readOnly {
		slog.WarnContext(ctx, "rekor is in read-only mode, cannot create new entry")
		_ = grpc.SetHeader(ctx, metadata.Pairs(httpStatusCodeHeader, "405"))
